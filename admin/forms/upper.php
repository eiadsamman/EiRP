...
<body class="theme-default <?= isset($themeDarkMode) ? $themeDarkMode->mode : ""; ?>" data-mode="<?= isset($themeDarkMode) ? $themeDarkMode->mode : ""; ?>">
	...
	<span class="header-ribbon noprint"><div><div class="btnheader-set" style="white-space:nowrap"><?php
	...
	if ($app->user->logged) {
		...
		echo "<a href=\"{$fs()->dir}/?--sys_sel-change=account\" tabindex=\"-1\" title=\"Running Account\" id=\"jqroot_sec\">" . (isset($app->user->account) ? "<span class=\"mediabond-hide\"></span>" . (!is_null($app->user->account->term) ? "{$app->user->account->term->termType()}: {$app->user->account->term->name}: " : "") . "{$app->user->account->name}" : "N/A") . "</a>";
	...
	?></div></div></span>
	<span id="header-menu" class="header-menu lefthand"><div><div><header><span class="btn-set"><input type="text" class="flex" id="PFSelector" data-slo=":LIST" data-source="_/MenuListItems/slo/<?= md5("#Fg32-32-f-" . ($app->user->info->id)); ?>/slo_listitems.a" placeholder="Goto page..." /></span></header><div style="white-space:nowrap;" class="menu-items" data-chunk_source="_/MenuListItems/html/<?= md5("#Fg32-32-f-" . ($app->user->info->id)); ?>/menu_listitems.a" data-content_type="html"></div></div></div></span>
	<span id="account-menu" class="header-menu righthand"><div><div><header><span class="btn-set"><input type="text" class="flex" id="account-menu-slo" data-url="<?= $fs()->dir ?>" data-list="accounts-list" data-slo=":LIST" placeholder="Select an account..."></span><datalist id="accounts-list"><?php
		if ($app->user->company) {
		...
		?></datalist></header><div style="white-space:nowrap;" class="menu-items" id="menu-account-selection"><?php
		if (!$app->user->company) {
	...
	?></div></div></div></span>
	<span id="company-menu" class="header-menu righthand"><div><div><header><span class="btn-set"><input type="text" class="flex" id="company-menu-slo" data-url="<?= $fs()->dir ?>" data-slo=":LIST" data-list="company-list" placeholder="Select a company..."></span><datalist id="company-list"><?= $SmartListObject->userCompanies(); ?></datalist></header><div style="white-space:nowrap;" class="menu-items" id="menu-company-selection"><?php
		$r = $app->db->query(
			"SELECT 
			comp_id,comp_name 
		FROM companies 
	...
	?></div></div></div></span>
	<span id="user-menu" class="header-menu righthand"><div><div><div style="white-space:nowrap;" class="menu-items"><?php
