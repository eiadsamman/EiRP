...
class SimpleImage {
	...
	function load($filename) {
		if($image_info = getimagesize($filename)){}else{return false;};
		$this->image_type = $image_info[2];
		$this->image_file_path = $filename;
		if( $this->image_type == IMAGETYPE_JPEG ) {
			if($this->image = imagecreatefromjpeg($filename)){}else{return false;};
		} elseif( $this->image_type == IMAGETYPE_GIF ) {
			if($this->image = imagecreatefromgif($filename)){}else{return false;};
		} elseif( $this->image_type == IMAGETYPE_PNG ) {
			if($this->image = imagecreatefrompng($filename)){}else{return false;};
	...
	function crop($width,$height,$horizontal,$vertical,$arrColors=false) {
		$new_image = @imagecreatetruecolor($width, $height) or die(false);
		if($arrColors==false){
			$bg =imagecolorat($this->image,0,0);
			
		}else{
			if(sizeof($arrColors)!=3){return false;}
		...
		imagecopyresampled($new_image, $this->image, 0, 0, $hor, $ver, $width, $height, $width, $height);
