...
class Accounting extends System
{
	...
	{
		if (!static::$sql) {
			throw new Exception("Database connection failed", 0);
		}
	}
	...
	))
	{
		...
		if (preg_match("/^([0-9]{4})-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])$/", $param['date'], $match)) {
			if (checkdate($match[2], $match[3], $match[1])) {
				$_param['date'] = mktime(0, 0, 0, $match[2], $match[3], $match[1]);
			}
		}
		...
		$r_checkCategory = static::$sql->query("SELECT acccat_id FROM acc_categories WHERE acccat_id={$_param['category']};");
		...
		$__workingaccount = $this->account_information(static::$_user->account->id);
		...
		$result &= static::$sql->query($qacc_main);
		if ($result) {
			$mainid = static::$sql->insert_id();
			...
			$result &= static::$sql->query($qacc_release);
			if (!$result) {
				static::$sql->rollback();
			...
			$result &= static::$sql->query($qacc_insert);
			if (!$result) {
				static::$sql->rollback();
				throw new Exception($this->__err[7301105], 7301105);
			}
	...
	public function get_currency_list()
	{
		...
		$temp = array();
		if ($r = static::$sql->query("SELECT cur_id,cur_name,cur_symbol,cur_default,cur_shortname FROM currencies;")) {
	...
	public function account_information(int $accountId)
	{
		...
		if ($r = static::$sql->query("
			...
			FROM 
				`acc_accounts` 
					JOIN user_partition ON prt_id=upr_prt_id AND upr_usr_id=" . static::$_user->info->id . " 
	...
	{
		$temp = false;
		if ($r = static::$sql->query("SELECT cur_id,cur_name,cur_symbol,cur_shortname FROM currencies WHERE cur_default=1;")) {
			if ($row = static::$sql->fetch_assoc($r)) {
				$temp = array(
					"id" => $row['cur_id'],
					"name" => $row['cur_name'],
