 ...
 * v2.18.11 181114
 */
class AttendanceList extends SQL {
	private $_dump=false;
	...
	**/
	private function FillGaps($from,$to,$prt,$ltrid,$latefinish,$earlystart,$closerid,$status,$maincheckinOwner,$CutDownStart=false){
		...
		while($sdate<=$fdate && $cnt<=31){
			if(!isset($this->_arratt[$smonth])){$this->_arratt[$smonth]=array();}
			if(!isset($this->_arratt[$smonth][$sdate])){$this->_arratt[$smonth][$sdate]=array();}
			$time=date("His",$from);
			
			$this->_arratt[$smonth][$sdate][$time]=array(
				/*0:StartTime*/		$from,					/*1:FinishTime*/	0,
			...
			if($sdate!=$fdate){
				$this->_arratt[$smonth][$sdate][$time][1]=mktime(23,59,59,date("m",$from),date("d",$from),date("Y",$from));
			}else{
				$this->_arratt[$smonth][$sdate][$time][1]=$to;
			}
			$from=mktime(0,0,0,date("m",$from),date("d",$from)+1,date("Y",$from));
			$smonth=date("Y-m",$from);
			$sdate=date("Y-m-d",$from);
			$cnt++;
		}
	...
	private function filldays(){
		...
		while($currentTime<=$this->_dateTo[2]){
			$scurrent=mktime(0,0,0,date("m",$currentTime),date("d",$currentTime),date("Y",$currentTime));
			$fcurrent=mktime(23,59,59,date("m",$currentTime),date("d",$currentTime),date("Y",$currentTime));
			...
			foreach($monthv as $datek=>$datev){
				...
				foreach($datev as $k=>$v){
					if($temp==null && date("His",$v[0])!="000000"){
						$this->_arratt[$monthk][$datek]['000000']=array(
							/*0:StartTime*/	mktime(0,0,0,date("m",$v[0]),date("d",$v[0]),date("Y",$v[0])),	/*1:FinishTime*/	$v[0],
				...
				if(date("His",$temp[1])!="235959"){
					$this->_arratt[$monthk][$datek][date("His",$temp[1])]=array(
						$temp[1],mktime(23,59,59,date("m",$temp[1]),date("d",$temp[1]),date("Y",$temp[1])),
	...
	private function GetRegistrationDate(){
		$r=$this->query("SELECT UNIX_TIMESTAMP(lbr_registerdate) AS lbr_registerdate FROM labour WHERE lbr_id={$this->_user_id};");
		if($r){
	...
	private function GetResignationDate(){
		$r=$this->query("SELECT UNIX_TIMESTAMP(lbr_resigndate) AS lbr_resigndate FROM labour WHERE lbr_id={$this->_user_id};");
		if($r){
			if($row=$this->fetch_assoc($r)){
	...
	**/
	public function getAttendaceList($userid,$dateFrom,$dateTo,$filldays=false,$consider_last_checking=false){
		...
		if($ra=$this->query("
			...
			;")){
			while($rowa=$this->fetch_assoc($ra)){
				...
				if(!isset($this->_arratt[$month])){$this->_arratt[$month]=array();}
				if(!isset($this->_arratt[$month][$date])){$this->_arratt[$month][$date]=array();}
				
				$this->_arratt[$month][$date][date("His",$rowa['_tstart'])]=array(
						/*0:StartTime*/		$rowa['_tstart'],
				...
				$this->FillGaps(
					/*FromTime*/$rowa['_tstart'],
					/*ToTime*/$rowa['_tend'],
					/*SectorID*/$rowa['prt_id'],
					/*OpenerID*/$rowa['ltr_id'],
					/*LateFinish*/$rowa['_tend'],
	...
	**/
	public function formatTime($time){
		$neg=$time<0;
		$time=abs($time);
		return ($neg?"(":""). str_pad(floor(($time)/60/60),2,"0",STR_PAD_LEFT).":".str_pad(floor(($time)/60%60),2,"0",STR_PAD_LEFT) . ($neg? ")" : "");
	}
		...
		foreach( $this->_arratt as $monthk=>$monthv){
			...
			if(preg_match("/^([0-9]{4})-([0-9]{2})$/i",$monthk,$matches)){
				if(checkdate($matches[2],1,$matches[1])){
					$month_name=date("Y, F",mktime(0,0,0,$matches[2],1,$matches[1]));
				}
			}
