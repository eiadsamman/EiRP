...
$settings = new Settings("{$_SERVER['FILE_SYSTEM_ROOT']}settings.json");
...
if (!$session->mysession_start("CPANEL3")) {
	ob_end_clean();
	header('HTTP/1.0 454 Session Not Found (RTSP)');
	exit;
}
...
//Handle errors
//$error = new ErrorHandler($_SERVER['FILE_SYSTEM_ROOT'] . $c__settings['site']['errorlogfile']);
