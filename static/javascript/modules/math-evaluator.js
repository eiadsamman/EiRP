export default class MathEvaluator {
	constructor(filed) {
		this._symbols = {};
		...
		this.defineOperator("pi", Math.PI);

		const that = this;
		this.inputField = filed;
		if (this.inputField) {
			this.inputField.addEventListener('input', () => { this.perform(); });
			this.inputField.addEventListener('change', () => { this.perform(); });
			this.perform();
		}
	}
	perform() {
		if (this.inputField.dataset.busy) return;
		const result = this.calculate(this.inputField.value);
		if (isNaN(result) || result === "" || result === Infinity) {
			delete this.inputField.dataset.eval;
			//this.inputField.style.borderColor = "red";
		} else {
			this.inputField.dataset.eval = result;
			//this.inputField.style.borderColor = "hsl(206, 100%, 46%)";
		}
	}
