...
if (isset($_GET['modify-user'], $_GET['token']) && $_GET['token'] == session_id()) {
	...
	if ($modify_user->loaded && isset($_POST['invoke'])) {

		if (isset($_POST['relative']) && $_GET['modify-user'] == $_POST['relative'] && $_POST['invoke'] == "info") {
		...
		if (isset($_POST['id'], $_GET['modify-user'], $_POST['relative'], $_GET['token']) && $_GET['token'] == session_id() && $_GET['modify-user'] == $_POST['relative'] && $_GET['modify-user'] == $modify_user->info->id && $_POST['invoke'] == "add-company") {
			$id = (int) $_POST['id'];
		...
		if (isset($_GET['modify-user'], $_POST['relative'], $_GET['token']) && $_GET['token'] == session_id() && $_GET['modify-user'] == $_POST['relative'] && $_GET['modify-user'] == $modify_user->info->id) {
			if ($_POST['invoke'] == "accounts") {
...
<div
	...
	<form action="<?= $fs()->dir; ?>/" method="GET" id="frmUserSelection">
		...
		<div class="btn-set">
			...
			<input type="text" data-slo="B001" class="flex"
				value="<?php echo isset($modify_user) && $modify_user->loaded ? $modify_user->info->username : ""; ?>" id="inpUserSelection"
					...
					<table>
						<tbody>
							<tr>
								...
								<td style="width:100%">
									<div class="btn-set"><label class="btn-checkbox"><input type="checkbox" name="active" <?= ($modify_user->info->active ? " checked=\"checked\" " : ""); ?> />
											<span>&nbsp;Active</span></label><input type="text" value="<?= $modify_user->info->id; ?>" class="flex"
							...
							<tr>
								<th>Username</th>
								<td>
									<div class="btn-set"><input type="text" placeholder="Username..." name="username" class="flex"
											value="<?= $modify_user->info->username; ?>" /></div>
								</td>
							</tr>
							<tr>
								<th>Password</th>
								<td>
									<div class="btn-set"><input type="password" placeholder="Password..." name="_password" class="flex" value="*****" />
									</div>
								</td>
							</tr>
							<?php if ($modify_user->info->id == $app->user->info->id) { ?>
								<tr>
									<th>Permissions</th>
									<td class="btn-set"><input type="text" value="<?= $app->permission($modify_user->info->permissions)->name; ?>"
											class="flex" readonly="readonly" disabled="disabled" /></td>
								</tr>
							<?php } else { ?>
								<tr>
									<th>Permissions</th>
									<td class="btn-set"><input type="text" name="perm" placeholder="Permissions..." id="ListObjectPerm" class="jQpermsel"
											...
											value="<?= $app->permission($modify_user->info->permissions)->name; ?>" data-slo="PERM_LEVEL" class="flex" /></td>
								</tr>
							<?php } ?>
						</tbody>
					</table>
