...
<div style="padding:20px 0px 10px 0px;min-width:300px;max-width:800px;background-color: #fff;position: sticky;top:43px;z-index: 50;">
	<form action="<?= $fs()->dir; ?>/" method="GET" id="frmUserSelection">
		<input type="hidden" name="modify-user" id="ModifyUser" value="<?php echo $modify_user->loaded ? $modify_user->info->id : ""; ?>" />
		<input type="hidden" name="token" value="<?php echo session_id(); ?>" />
		<div class="btn-set">
			<span>Employee Name \ ID</span><input type="text" data-slo="B001" class="flex" value="<?php echo $modify_user->loaded ? $modify_user->info->username : ""; ?>" id="inpUserSelection" placeholder="Select user..." /><button type="submit">Modify</button>
		</div>
	</form>
</div>

<div id="UserDetailts" style="min-width:300px;max-width:800px;position:relative;">
	<?php if ($modify_user->loaded) { ?>
					...
					<table class="bom-table">
						<tbody>
							<tr>
								<th>ID</th>
								<td style="width:100%">
									<div class="btn-set"><label class="btn-checkbox"><input type="checkbox" name="active" <?= ($modify_user->info->active ? " checked=\"checked\" " : ""); ?> /> <span>&nbsp;Active</span></label><input type="text" value="<?= $modify_user->info->id; ?>" class="flex" readonly="readonly" /></div>
								</td>
							</tr>
							<tr>
								<th>Username</th>
