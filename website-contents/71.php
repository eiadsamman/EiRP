...
SELECT 
	...
	prt_name,prt_id,prt_color,
	
	ltr_usr_id,
	_labor.lsc_name AS lsc_name,
	_labor.lsc_id AS lsc_id
FROM
	labour_track AS _major
		...
		JOIN
			`acc_accounts` ON prt_id=ltr_prt_id
WHERE
	ltr_type=1
	".($groupby=='hour'?"AND DATE_FORMAT(ltr_ctime,'%Y-%m-%d') = '".(date("Y-m-d",$hours))."'":"")."
	".($groupby=='day'?"AND DATE_FORMAT(ltr_ctime,'%Y-%m') = '".(date("Y-m",$month))."'":"")."
GROUP BY
	grpdate,lsc_id,prt_id
ORDER BY
	grpdate ASC,pcnt desc
")){
	while($row=$sql->fetch_assoc($r)){
		if(!isset($arrout[$row['lsc_id']])){$arrout[$row['lsc_id']]=array($row['lsc_name'],array(),array());}
		if(!isset($arrout[$row['lsc_id']][1][$row['grpdate']])){$arrout[$row['lsc_id']][1][$row['grpdate']]=array();}
		if(!isset($arrout[$row['lsc_id']][2][$row['prt_id']])){$arrout[$row['lsc_id']][2][$row['prt_id']]=$row['prt_name'];}
		$colorlist=array();
		$row['prt_color']=substr(str_pad($row['prt_color'],6,"0",STR_PAD_LEFT),0,6);
		$colorlist[0]=hexdec($row['prt_color'][0].$row['prt_color'][1]);
		$colorlist[1]=hexdec($row['prt_color'][2].$row['prt_color'][4]);
		$colorlist[2]=hexdec($row['prt_color'][4].$row['prt_color'][5]);
		if(!isset($arrout[$row['lsc_id']][3][$row['prt_id']])){$arrout[$row['lsc_id']][3][$row['prt_id']]=implode(",",$colorlist);}
		$arrout[$row['lsc_id']][1][$row['grpdate']][$row['prt_id']]=$row['pcnt'];
	}
}

foreach($arrout as $k=>$v){
	$arrout[$k][2]["cum"]="Total";
	$arrout[$k][3]["cum"]="255,60,0";
...
foreach($arrout as $cotk=>$cotv){
	...
	foreach($cotv[2] as $legk=>$legv){
		echo "<div><span style=\"background-color:rgba({$cotv[3][$legk]},1);\"></span>$legv</div>";
		$cnt++;
	}
...
$(document).ready(function(e) {
	var datesel = $("#date").slo({"limit":7});
	var yearsel = $("#year").slo({"limit":7});
	var mon1sel = $("#month").slo({"limit":7});
	var mon2sel = $("#month2").slo({"limit":7});
	<?php
	foreach($arrout as $cotk=>$cotv){
		...
		echo "var barChart{$cotk} = new Chart(ctx{$cotk}).Line({
			labels: [\"\",";
			$smart="";
			$even=false;$swap=3;$cnt=0;
			if(sizeof($cotv[1])>30){$even=true;}
			...
			datasets: [
				...
				foreach($cotv[2] as $legenedk=>$legenedv){
					...
					echo "{";
					echo "label:\"$legenedv\",";
					echo "fillColor: \"rgba({$cotv[3][$legenedk]},0.1)\",";
					echo "strokeColor: \"rgba({$cotv[3][$legenedk]},0.7)\",";
					echo "pointColor: \"rgba({$cotv[3][$legenedk]},1)\",";
