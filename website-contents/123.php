...
ORDER BY
	grpdate ASC
")){
	if(!isset($arrout[$reportid])){$arrout[$reportid]=array($reportpage['title'],array(),array());}
	while($row=$sql->fetch_assoc($r)){
		if(!isset($arrout[$reportid][1][$row['grpdate']])){$arrout[$reportid][1][$row['grpdate']]=array();}
		if(!isset($arrout[$reportid][2][$row['cot_id']])){$arrout[$reportid][2][$row['cot_id']]=$row['cot_name'];}
