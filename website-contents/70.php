...
")){
	while($row=$sql->fetch_assoc($r)){
		if(!isset($arrout[$row['ptp_id']])){$arrout[$row['ptp_id']]=array($row['ptp_name'],array(),array());}
		if(!isset($arrout[$row['ptp_id']][1][$row['grpdate']])){$arrout[$row['ptp_id']][1][$row['grpdate']]=array();}
		if(!isset($arrout[$row['ptp_id']][2][$row['prt_id']])){$arrout[$row['ptp_id']][2][$row['prt_id']]=$row['prt_name'];}
