...
if (isset($_POST['method']) && $_POST['method'] == 'addstatement') {
	...
	if (!$USER->account->role->inbound) {
		_JSON_output(false, "Inbound rules is not allowed on `{$USER->account->name}`");
	}
...
if (!$__workingaccount) {
...
} elseif (!$USER->account->role->inbound) {
	...
	$_TEMPLATE->NewFrameBody('<ul>
		...
		<ul>
			...
			<li>Goto <a href="' . $tables->pagefile_info(95, null, "directory") . '">New Payment</a></li>
		</ul>
		');
} else {
?>
