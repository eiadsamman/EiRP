...
if($r=$sql->query("
	...
	while($row=$sql->fetch_assoc($r)){
		...
		if(!isset($arrout[$reportid][1][$row['lsc_name']])){$arrout[$reportid][1][$row['lsc_name']]=$row['cnt'];}
		if(!isset($arrout[$reportid][2][$row['lsc_name']])){$arrout[$reportid][2][$row['lsc_name']]=is_null($row['lsc_color'])?"cccccc":$row['lsc_color'];}
		$total+=$row['cnt'];
	}
}
echo $sql->error();
