...
if(isset($_POST['method'],$_POST['job-id']) && $_POST['method']=="update-job-salary"){
	...
	$job_info=false;
	$q=$sql->query("SELECT lty_id,lty_name,lsc_name,lty_section FROM labour_type LEFT JOIN labour_section ON lsc_id=lty_section WHERE lty_id=$job_id ORDER BY lty_id;");
	if($q){
		if($row=$sql->fetch_assoc($q)){
			$job_info=array();
	...
	$_workingtimes=array();
	$q=$sql->query("SELECT lwt_id,lwt_name FROM workingtimes ORDER BY lwt_id;");
	if($q){
	...
	$q=$sql->query("SELECT lbr_mth_id,lbr_mth_name FROM labour_method;");
	if($q){
		while($row=$sql->fetch_assoc($q)){
			echo "<tr>";
			...
			foreach($_workingtimes as $_wt_id=>$_wt_val){
				...
				if($r_der_vals){
					if($row_der_vals=$sql->fetch_assoc($r_der_vals)){
...
echo "</thead><tbody>";
$q=$sql->query("SELECT lty_id,lty_name,lsc_name,lty_section FROM labour_type LEFT JOIN labour_section ON lsc_id=lty_section;");
if($q){
	while($row=$sql->fetch_assoc($q)){
