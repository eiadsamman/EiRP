...
<table class="bom-table">
	<thead><tr class="special"><td><h2><b>Account selecting</b></h2></td></tr></thead>
	<tbody>
...
<div id="screenSector">
<table class="bom-table">
	<thead><tr class="special"><td><h2>Select Account</h2></td></tr></thead>
	<tbody>
	<tr>
		...
		<?php
			...
			if($r=$sql->query("
				...
				while($row=$sql->fetch_assoc($r)){
					$accountfound=true;
					if(!isset($ptp[$row['comp_name']])){$ptp[$row['comp_name']]=array();}
					if(!isset($ptp[$row['comp_name']][$row['ptp_name']])){$ptp[$row['comp_name']][$row['ptp_name']]=array();}
