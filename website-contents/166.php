	...
	if($r){
		if($sql->num_rows($r)==0){
			...
			while($row=$sql->fetch_assoc($r)){
				if(!$firstrecord){$firstrecord=$row['curexglog_date'];}
...
$(document).ready(function(e) {
	...
	var method_slo={"month":["MONTH","Select month","Month"],"year":["YEAR","Select year","Year"]}
	...
	$(".menu_screen,.menu_screen ~ button.menu").on('mouseenter',function(){
		if(pageination_list_timer!=null){clearTimeout(pageination_list_timer);}
		$(".menu_screen > div").css("display","block");
	}).on('mouseleave',function(){
		pageination_list_timer=setTimeout(function(){$(".menu_screen > div").css("display","none");},500);
	});
	...
	$("#jQform").on('submit',function(e){
		...
		$.ajax({
			...
			if(json.result){
				...
				$("#jQoutput").html("");
				switch(json.field){
					case "from":messagesys.failure("Select exchange from currency");break;
					case "to":messagesys.failure("Select exchange to currency");break;
					case "date":messagesys.failure("Select date range");break;
					case "empty":messagesys.success("No records found for selected period");break;
				}
