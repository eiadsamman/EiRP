<table class="bom-table hover">
	...
	<tbody>
		...
		$r = $app->db->query(
			"SELECT 
				usrset_value,usrset_usr_defind_name,UNIX_TIMESTAMP(usrset_time) AS usrset_time 
			FROM 
				user_settings 
			WHERE 
		...
		if ($r) {
			if ($r->num_rows == 0) {
				...
				while ($row = $r->fetch_assoc()) {
					echo "<tr data-id=\"{$row['usrset_usr_defind_name']}\"><td style=\"width:10px;\" class=\"op-donwload jQload_query\"><span></span></td><td style=\"width:300px;\">" . $row['usrset_usr_defind_name'] . "</td>
