...
if (isset($_POST['emp_method'])) {
	...
	$arroutput = array(
		"result" => true,
		"source" => array(
			"firstname" => (isset($_POST['firstname']) && trim($_POST['firstname']) != "" ? addslashes(trim($_POST['firstname'])) : false),
			/*required*/
			"lastname" => (isset($_POST['lastname']) && trim($_POST['lastname']) != "" ? addslashes(trim($_POST['lastname'])) : false),
			/*required*/
			...
			/*required*/
			"gender" => (isset($_POST['gender'][1]) && (int) $_POST['gender'][1] != 0 ? (int) $_POST['gender'][1] : false),
			/*required*/
			...
			"payment" => (isset($_POST['payment'][1]) && (int) $_POST['payment'][1] != 0 ? (int) $_POST['payment'][1] : false),
			/*required*/
			"jobtitle" => (isset($_POST['jobtitle'][1]) && (int) $_POST['jobtitle'][1] != 0 ? (int) $_POST['jobtitle'][1] : false),
			/*required*/
			"regdate" => (isset($_POST['regdate'][1]) && check_date($_POST['regdate'][1]) ? $_POST['regdate'][1] : false),
			/*required*/
			"birthdate" => (isset($_POST['birthdate'][1]) && check_date($_POST['birthdate'][1]) ? $_POST['birthdate'][1] : false),
			/*required*/
			"nationality" => (isset($_POST['nationality'][1]) && (int) $_POST['nationality'][1] != 0 ? (int) $_POST['nationality'][1] : null),
			"company" => (isset($_POST['company'][1]) && (int) $_POST['company'][1] != 0 ? (int) $_POST['company'][1] : false),
			/*required*/
		)
	);
	...
	$LowestPermission = 0;
	$LowestPermissionQuery = $app->db->query("SELECT per_id FROM permissions WHERE per_order = (SELECT MIN(per_order) FROM permissions) LIMIT 1;");
	if ($LowestPermissionQuery) {
		if ($LowestPermissionRow = $LowestPermissionQuery->fetch_assoc()) {
	...
	if ($arrparser['result']) {
		...
		if ($app->db->query($q)) {
			...
			if (
				...
				if (!$app->db->query("UPDATE uploads SET up_rel=$userid WHERE up_id IN (" . implode(",", array_merge($arroutput['source']['perosnal_image'], $arroutput['source']['social_id_image'])) . ");")) {
					$arrparser['result'] = false;
			...
			if ($arrparser['result']) {
				...
				if (!$app->db->query($q_labour_insert)) {
					$arrparser['result'] = false;
					$arrparser['source']["global"] = "Failed to insert the new employee records!" . addslashes($app->db->error);
				}
			}
		} else {
			$arrparser['result'] = false;
...
<style>
	...
	.custome>tbody>tr>td>input,
	.custome>tbody>tr>td>span.cssSLO_wrap>input {
		max-width: 500px;
		width: 500px;
		min-width: 500px;
	}
...
<?php if ($debug) { ?>
	<div class="btn-set"><textarea id="__debug" style="width:100%;height:100px;margin-bottom:20px;" readonly="readonly"
			placeholder="Debuger"></textarea></div>
<?php } ?>
...
<form id="jQform">
	...
	<table class="bom-table custome">
		<tbody>
			...
			<tr>
				<th>Nationality</th>
				<td class="btn-set"><input type="text" name="nationality" id="slonationality" data-slo="COUNTRIES"
						value="" data-slodefaultid=""></td>
			...
			<tr>
				...
				<td>
					<div class="btn-set" style="max-width: 500px;">
						<input type="type" name="social_number"
						...
						<button type="button" id="js_upload_trigger_1" class="js_upload_trigger"
							data-db_rel="usr_attrib_s2">ID Card photo</button>
				...
				<td><span id="jserr_social_number" class="required_field"></span><span id="jserr_social_id_image"
			...
			<tr>
				...
				<td>
					<div class="btn-set" style="max-width: 500px;">
						<button type="button" id="js_upload_trigger"
							style="-webkit-box-flex: 1;-moz-box-flex: 1;-webkit-flex: 1;-ms-flex: 1;flex: 1;"
							class="js_upload_trigger">Upload perosnal photo</button>
			...
			<tr>
				<th>Gender</th>
				<td class="btn-set"><input type="text" name="gender" id="slogender" data-slo="G000" value=""
						data-slodefaultid=""></td>
				<td id="jserr_gender" class="required_field"></td>
			</tr>
			<tr>
				<th>Birthdate</th>
				<td class="btn-set"><input type="text" name="birthdate" id="slobirthdate" data-slo="BIRTHDATE" value=""
						data-slodefaultid=""></td>
			...
			<tr>
				<th>Residence</th>
				<td class="btn-set"><input type="text" name="residence" id="sloresidence" data-slo="E004" value=""
						data-slodefaultid=""></td>
				<td id="jserr_residence" class="required_field"></td>
			</tr>
			<tr>
				<th>Transportation</th>
				<td class="btn-set"><input type="text" name="transportation" id="slortransportation"
						data-slo="TRANSPORTATION" value="" data-slodefaultid=""></td>
		...
		<tr>
		 ...
		 -->
			...
			<tr>
				...
				<td class="btn-set"><input type="text" name="regdate" id="sloregdate" data-slo="DATE"
						value="<?php echo date("Y-m-d"); ?>" data-slodefaultid="<?php echo date("Y-m-d"); ?>" /></td>
				<td id="jserr_regdate" class="required_field"></td>
			</tr>
			<tr>
				<th>Job title</th>
				<td class="btn-set"><input type="text" name="jobtitle" id="slotype" data-slo="E002A" value=""
						data-slodefaultid=""></td>
				<td id="jserr_jobtitle" class="required_field"></td>
			</tr>
			<tr>
				<th>Working shift</th>
				<td class="btn-set"><input type="text" name="shift" id="sloshift" data-slo="E003" value=""
						data-slodefaultid=""></td>
				<td id="jserr_shift" class="required_field"></td>
			</tr>
			<tr>
				<th>Working Time</th>
				<td class="btn-set"><input type="text" name="workingtimes" id="sloworkingtimes" data-slo="WORKING_TIMES"
						value="" data-slodefaultid=""></td>
				<td id="jserr_workingtimes" class="required_field"></td>
			</tr>
			<tr>
				<th>Payment method</th>
				<td class="btn-set"><input type="text" name="payment" id="slopayment" data-slo="SALARY_PAYMENT_METHOD"
						value="" data-slodefaultid=""></td>
			...
			<tr>
				<td colspan="3">
					<div class="btn-set center"><button type="button" id="jQbtnSubmit">Add Employee</button></div>
				</td>
			</tr>
		</tbody>
	</table>
...
<script>
	...
	$(document).ready(function (e) {
		UploadUserPersonalImage = $.Upload({
			...
			relatedpagefile: <?= $app->scope->individual->portrait; ?>,
			multiple: false,
