...
<style>
	...
	div.printerform > div{
		...
		background:
			url("<?= $_SERVER['HTTP_SYSTEM_ROOT'];?>/idcard-bg.jpg") 0 100%  / auto 100% no-repeat,
	...
	div.printerform > div > div.logo{
		...
		width: 100%;
		height: 35px;
		background-image: url("<?= $_SERVER['HTTP_SYSTEM_ROOT'];?>download/?id=<?= $USER->company->logo;?>&pr=t");
...
if($r=$sql->query("
	...
	while($row=$sql->fetch_assoc($r)){
			...
			if(!is_null($row['up_id']) && (int)$row['up_id']!=0){
				$personalPhoto=" style=\"background-image:url('{$_SERVER['HTTP_SYSTEM_ROOT']}".$tables->pagefile_info(187,null,"directory")."?id={$row['up_id']}&pr=v')\"";
			}else{
				$personalPhoto=" style=\"background-image:url('{$_SERVER['HTTP_SYSTEM_ROOT']}/user.jpg')\"";
			}
			...
			echo "<div>";
				...
				echo "";
				echo "<div class=\"serial\"><img src=\"{$_SERVER['HTTP_SYSTEM_ROOT']}".$tables->pagefile_info(15,null,"directory")."/?c={$row['usr_id']}&f=1&t=13\" /></div>";
