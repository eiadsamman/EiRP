...
if($h__requested_with_ajax && isset($_POST['vdocid'],$_POST['token'])){
	...
	$rpo=$sql->query("SELECT po_id,po_comp_id,po_costcenter FROM inv_main WHERE po_id=$post_doc_id AND po_type=".Invoice::map['PUR_QUT']." AND po_close_date IS NULL");
	...
	$prev_doc['remarks']=trim($sql->escape($_POST['vremarks']));
	...
	$rwo=("INSERT INTO inv_main (	po_serial,po_type,po_shipto_acc_id,po_billto_acc_id,po_usr_id,
			...
			SELECT 
				$doc_serial,".Invoice::map['PUR_ORD'].",po_shipto_acc_id,po_billto_acc_id,{$USER->info->id},
				NOW(),NULL,NULL,
	...
	if($rwo){
		...
		$rwosq=$sql->query("UPDATE inv_main SET po_close_date = NOW() WHERE po_id = {$chain[0]} OR po_id = {$chain[1]}");
		if(!$rwosq){
			$sql->rollback();
	...
	try {
		...
		echo "<table class=\"bom-table\">
					...
					<tbody id=\"jQmaterialList\">";
						$sqlquery_materialList = $invoice->DocGetMaterialList($doc_rfq['po_id']);
		...
		if(is_null($doc_mr['po_close_date'])){
			...
			if($rrfq=$invoice->GetDocChildren($chain[0])){
				while($rowrfq=$sql->fetch_assoc($rrfq)){
					echo "<tr>";
					...
					if($rpo=$invoice->GetDocChildren($rowrfq['po_id'])){
						while($rowpo=$sql->fetch_assoc($rpo)){
							echo "<tr>";
		...
		$_TEMPLATE->NewFrameBody('<ul>
			...
			<br />Return to <a href="'.$tables->pagefile_info(237,null,"directory").'">Purhcase Orders</a>
			<br /><br />Place a new <a href="'.$tables->pagefile_info(230,null,"directory").'">Material Request</a>
			');
		...
		$("#jQbuttonSubmit").on("click",function(){
			...
			$.ajax({
				...
				if(response=="INERR"){
					...
					Template.PageRedirect("<?php echo $tables->pagefile_info(237,null,"directory");?>" + o,"<?php echo "{$c__settings['site']['title']} - ".$tables->pagefile_info(237,null,"title");?>",true);
