		...
		try {
			...
			if ($transaction->post()) {
				...
				if (!empty($_POST['party'])) {
					$tl->register(Module::CRMCustomer, Action::FinancePayment, (int)$_POST['party'][1], ["id" => $transaction->insert_id, "value" => number_format((float) $_POST['value'], 2, ".", ",") . " " . $app->user->account->currency->shortname]);
				}
			} else {
				$result['errno'] = 300;
