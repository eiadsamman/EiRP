...
$invoice 	= new Invoice();
$accounting = new Accounting($USER->info->permissions);
...
try {
	...
	echo "<a style=\"color:#333;\" href=\"".$tables->pagefile_info(240,null,"directory")."/?docid={$chain[0]}&token=".md5("sysdoc_".$chain[0].session_id())."\">{$doc_rm['doc_id']}</a>";
	echo "<a style=\"color:#333;\" href=\"".$tables->pagefile_info(234,null,"directory")."/?docid={$chain[1]}&token=".md5("sysdoc_".$chain[1].session_id())."\">{$doc_rfq['doc_id']}</a>";
	echo "<a style=\"color:#333;\" href=\"".$tables->pagefile_info(237,null,"directory")."/?docid={$chain[2]}&token=".md5("sysdoc_".$chain[2].session_id())."\">{$doc_po['doc_id']}</a>";
	...
	<script type="text/javascript">
		$(document).ready(function(e) {
			var jQGIdest	=$("#jQGIdest").slo({'limit':10,onselect:function(){}}).setparam({"company":<?php echo (int)$USER->company->id;?>});
			var $jQGIsource	=$("#jQGIsource").slo({'limit':10,onselect:function(){}}).setparam({"company":<?php echo (int)$doc_po['po_benf_comp_id'];?>});
