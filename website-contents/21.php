...
")){
	while($row=$sql->fetch_assoc($r)){
		if(!isset($arrout[$row['cot_id']])){$arrout[$row['cot_id']]=array($row['cot_name'],array(),array());}
		if(!isset($arrout[$row['cot_id']][1][$row['grpdate']])){$arrout[$row['cot_id']][1][$row['grpdate']]=array();}
		if(!isset($arrout[$row['cot_id']][2][$row['prt_id']])){$arrout[$row['cot_id']][2][$row['prt_id']]=$row['prt_name'];}
