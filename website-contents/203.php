...
if(isset($_POST['wosubmit'])){
	...
	if(!isset($_POST['wo_site'][1]) || (int)$_POST['wo_site'][1]==0){
		$output['msg'][]="Select work order site";echo json_encode($output);exit;
	}
	if($_POST['wo_title']==""){
		$output['msg'][]="Type in work order title";echo json_encode($output);exit;	
	}
	if(!isset($_POST['wo_item']) || !is_array($_POST['wo_item']) || sizeof($_POST['wo_item'])==0){
		$output['msg'][]="Select work order materials";echo json_encode($output);exit;
	}
	...
	if($rwo){
		...
		if($rwol){
			...
			$sql->rollback();
			$output['msg'][]="Creating work order failed!";echo json_encode($output);exit;
		}
	}else{
		$sql->rollback();
		$output['msg'][]="Creating work order failed!";echo json_encode($output);exit;
...
<table class="bom-table">
	...
	<tbody>
		<tr>
			<th>Project Title</th>
			<td width="100%"><div class="btn-set" style="max-width: 500px;"><input style="width: 100%" name="wo_title" value="test<?php echo date("dHis");?>" type="text" /></div></td>
		</tr>
		<tr>
			<th>Production Plant</th>
			<td><div class="btn-set" style="max-width: 500px;"><input data-slo="ACC_788" name="wo_site" id="jQdestinatoin" style="width: 100%" type="text" /></div></td>
		</tr>
		<tr>
			<th>Process Documents</th>
			<td><div class="btn-set" style="max-width: 500px;"><input style="width: 100%" name="wo_files" type="text" /></div></td>
		</tr>
		<tr>
			<th>Due Date</th>
			<td><div class="btn-set" style="max-width: 500px;"><input style="width: 100%" name="wo_due" type="text" /></div></td>
		</tr>
		<tr>
			<th>Responsible Manager</th>
			<td><div class="btn-set" style="max-width: 500px;"><input style="width: 100%" name="wo_manager" type="text" data-slo="B00S" id="jQinpMan" /></div></td>
		</tr>
		<tr>
			<th>Remarks</th>
			<td><div class="btn-set" style="max-width: 500px;"><textarea style="width: 100%;height:100px" name="wo_remarks"></textarea></div></td>
		</tr>
		<tr>
			<td colspan="2"><div class="btn-set center"><button id="jQpostSubmit" type="button">Submit new work order</button></div></td>
		</tr>
	</tbody>
</table>
<table class="bom-table hover" style="margin-top: 15px;">
	<thead>
		...
		<tr>
			<td></td>
			<td><div class="btn-set "><input id="jQbomqty" type="text" style="width:100px;text-align: right" /></div></td>
			<td colspan="5" style="color:#333"><div class="btn-set "><input id="jQbomselector" data-slo="BOM" style="-webkit-box-flex: 1;-moz-box-flex: 1;-webkit-flex: 1;-ms-flex: 1;flex: 1;" type="text" /></div></td>
...
<script type="text/javascript">
	...
	$(function(){
		$("#jQdestinatoin").slo({limit:10});
		$("#jQinpMan").slo({limit:10});
		...
		$(popup.self()).on("click","#jQpopBtnSubmit",function(){
			$.ajax({
				url:"<?php echo $tables->pagefile_info(203)['directory'];?>",
				type:"POST",
		...
		var DisplayBOM=function(mat,qty){
			$.ajax({
				url:"<?php echo $tables->pagefile_info(203)['directory'];?>",
				type:"POST",
				data:{"method":"displaybom","id":mat,"qty":qty}
		...
		$("#jQaddItem").on("click",function(){
			...
			$.ajax({
				url:"<?php echo $tables->pagefile_info(203)['directory'];?>",
				type:"POST",
				data:{"method":"checkitem","id":$jQbomSelector.hidden[0].val(),"qty":$jQbomQuantity.val()}
			}).done(function(output){
				try{json=JSON.parse(output);}catch(e){messagesys.failure("Parsing output failed");return false;}
				if(json.result==2){
					$newrow="";
