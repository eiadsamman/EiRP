...
GROUP BY
	grpdate,prt_id,lsc_id
ORDER BY
	grpdate ASC
")){
	while($row=$sql->fetch_assoc($r)){
		if(!isset($arrout[$row['prt_id']])){$arrout[$row['prt_id']]=array($row['prt_name'],array(),array());}
		if(!isset($arrout[$row['prt_id']][1][$row['grpdate']])){$arrout[$row['prt_id']][1][$row['grpdate']]=array();}
		if(!isset($arrout[$row['prt_id']][2][$row['lsc_id']])){$arrout[$row['prt_id']][2][$row['lsc_id']]=$row['lsc_name'];}
		$arrout[$row['prt_id']][1][$row['grpdate']][$row['lsc_id']]=$row['pcnt'];
	}
}
