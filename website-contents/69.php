...
")) {
	while ($row = $r->fetch_assoc()) {
		if (!isset($arrout[$row['prt_id']])) {
			$arrout[$row['prt_id']] = array($row['prt_name'], array(), array());
		}
		if (!isset($arrout[$row['prt_id']][1][$row['grpdate']])) {
			$arrout[$row['prt_id']][1][$row['grpdate']] = array();
		}
		if (!isset($arrout[$row['prt_id']][2][$row['lsc_id']])) {
			$arrout[$row['prt_id']][2][$row['lsc_id']] = $row['lsc_name'];
		}
