...
if(isset($_POST['new-child']) && isset($_POST['parent'])){
	...
	if($parent==0){
		...
		$r=$sql->query("SELECT hir_name FROM job_hierarchy WHERE hir_id=$parent;");
...
if(isset($_POST['display-child']) && isset($_POST['id'])){
	...
	if($parent==0){
		...
		$r=$sql->query("SELECT hir_name,hir_id,hir_parent FROM job_hierarchy WHERE hir_id=$parent;");
		if($r && $row=$sql->fetch_assoc($r)){
			$record=$row;
		}else{
			echo "Invalid branch";
	...
	if($r=$sql->query("SELECT jhr_hir_id,jhr_job_id,lty_name,lsc_name 
...
if(isset($_POST['add-job']) && isset($_POST['branch']) && isset($_POST['job'])){
	...
	if($sql->query("INSERT INTO job_hierarchyroles (jhr_hir_id,jhr_job_id) VALUES ({$_POST['branch']},{$_POST['job']});")){
		echo "1";
	}else{
		echo "0";
	}
...
if(isset($_POST['delete-job-role']) && isset($_POST['branch']) && isset($_POST['job'])){
	...
	$r=true;
	$r&=$sql->query("DELETE FROM job_hierarchyroles WHERE jhr_hir_id={$_POST['branch']} AND jhr_job_id={$_POST['job']};");
	if($r){
		$sql->commit();
...
if(isset($_POST['update-branch-name']) && isset($_POST['id']) && isset($_POST['name'])){
	...
	$json=new JSON();
	if($r=$sql->query("UPDATE job_hierarchy SET hir_name='{$_POST['name']}' WHERE hir_id={$_POST['id']}")){
		$json->output(true,"Branch name udpated successfully",null,array("id"=>$_POST['id'],"name"=>$_POST['name']));
	}else{
		$json->output(false,$sql->errno());
	}
	exit;
}
...
if(isset($_POST['delete-branch']) && isset($_POST['id'])){
	...
	$json=new JSON();
	$arrchilds=array();
	function digChilds($id,&$arrchilds){
		global $sql;
		$r=$sql->query("SELECT hir_id FROM job_hierarchy WHERE hir_parent=$id");
		if($r){
			while($row=$sql->fetch_assoc($r)){
				$arrchilds[]=$row['hir_id'];
				digChilds($row['hir_id'],$arrchilds);
			}
		}
	}
	
	$r=$sql->query("SELECT hir_id FROM job_hierarchy WHERE hir_parent={$_POST['id']}");
	if($r){
		while($row=$sql->fetch_assoc($r)){
			$arrchilds[]=$row['hir_id'];
			digChilds($row['hir_id'],$arrchilds);
	...
	foreach($arrchilds as $v){
		$r&=$sql->query("DELETE FROM job_hierarchy WHERE hir_id=$v");
		$r&=$sql->query("DELETE FROM job_hierarchyroles WHERE jhr_hir_id=$v");
	}
	if($r){
		$sql->commit();
...
if(isset($_POST['append-child']) && isset($_POST['parent']) && isset($_POST['name'])){
	...
	$r=$sql->query("INSERT INTO job_hierarchy (hir_parent,hir_name) VALUES ($parent,'$name');");
	if($r){
		$newid=$sql->insert_id();
		echo $json->output(true,"Child added successfully",null,array("parent"=>$parent,"name"=>$name,"id"=>$newid));
	}else{
		echo $json->output(false,$sql->errno());
	}
	exit;
}
...
if(isset($_POST['add-lbr']) && isset($_POST['branch']) && isset($_POST['lbr'])){
	...
	if($r=$sql->query("INSERT INTO job_hierarchylabour (hlb_jhr_id,hlb_lbr_id) VALUES ({$_POST['branch']},{$_POST['lbr']});")){
		$newid=$sql->insert_id();
...
if(isset($_POST['delete-employee-hir']) && isset($_POST['lbr'])){
	...
	$_POST['branch']=(int)$_POST['branch'];
	$json=new JSON();
	if($sql->query("DELETE FROM job_hierarchylabour WHERE hlb_lbr_id={$_POST['lbr']} AND hlb_jhr_id={$_POST['branch']}")){
...
$(document).ready(function(e) {
	...
	$("#css_hierLeft").on('click',".jQplot",function(e){
		...
		$.ajax({
			...
			$("#jQhierBody").html(data);
			$("#jQhierBody").find("#jQjobRole").slo({limit:10,
			...
			$("#jQhierBody").find("#jQemployee").slo({
				...
				onselect:function(output){
					...
					$.ajax({
						...
						}catch(e){messagesys.failure("Unable to fetch output, refresh the page");return;}
						if(json.result){
