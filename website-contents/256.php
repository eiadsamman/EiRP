...
$expirydate=false;
$rexp = $sql->query("SELECT UNIX_TIMESTAMP(lbr_resigndate) AS rdate FROM labour WHERE lbr_id = {$USER->info->id};");
if($rexp && $rrow = $sql->fetch_assoc($rexp)){
				...
				<div style="position: sticky;top: 111px;">
					...
					<table class="bom-table">
							...
							<tr style="display:none">
								<td colspan="2"><div class="btn-set"><button id="js-saveimage">Save image</button><button id="js-clearall">Clear</button></div></td>
							</tr>
		...
		let Update = function () {
			...
			for(let i=0;i<DOMInvoke.length;i++){
				try{
					...
					if(DOMInvoke[i][1]!=null){DOMInvoke[i][1].style.display = 'none';}
					DOMInvoke[i][0].style.background = "#ffffff";
				}catch (e){
					confirminput&=false;
					if(DOMInvoke[i][1]!=null){DOMInvoke[i][1].style.display = 'block';}
				}
			}
			if(confirminput)
				p.Process();
