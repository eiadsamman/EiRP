...
if($h__requested_with_ajax && isset($_POST['vdocid'],$_POST['token'])){
	...
	$prev_doc['remarks']			=$sql->escape($_POST['vremarks']);
...
if($doc_id){
	try {
		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				...
				$("#jQinputCompany").slo({
					'onselect':function(o){
						slocontact.setparam({"slocompany":o.hidden});
					},
					'ondeselect':function(){
						slocontact.setparam({"slocompany":0});
						slocontact.clear();
					}
				}).setparam({"sloexcludecompany":"<?php echo (int)$rowpo['po_comp_id'];?>"});
				...
				$("#jQbuttonSubmit").on("click",function(){
					...
					$.ajax({
						...
						if(response=="INERR"){
							...
							Template.PageRedirect("<?php echo $tables->pagefile_info(234,null,"directory");?>" + o,"<?php echo "{$c__settings['site']['title']} - ".$tables->pagefile_info(234,null,"title");?>",true);
	...
	$_TEMPLATE->Title("&nbsp;Not Found!", null, "","mark-error");
		...
		$_TEMPLATE->NewFrameBody('<ul>
			...
			<b>Actions</b>
			<ul>
				<li>Return to <a href="'.$tables->pagefile_info(234,null,"directory").'">Request for Quotation</a></li>
				<li>Goto to <a href="'.$tables->pagefile_info(240,null,"directory").'">material requests</a></li>
			</ul>
			');
}
?>