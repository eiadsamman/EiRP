...
function SidePanelContent($sql, $user, $pageUrl, $pageTitle, $newUrl, $newTitle){
	...
	if($mysqli_result){
		if($sql->num_rows($mysqli_result)==0){
			echo "<div><span>No pending quotations</span></div>";
		}
		while($mysqli_record = $sql->fetch_assoc($mysqli_result)){
			echo "<div>";
					...
					foreach($v as $elem_key => $elem_val){
						...
						echo "<a class=\"role-templatelink\" data-role_title=\"{$pageTitle}\" href=\"{$pageUrl}/?docid={$elem_val['po_id']}&token=".md5("sysdoc_".$elem_val['po_id'].session_id())."\">
							...
							<span><span>".(number_format( 
								$doc_value
							 ,2,".",",")).$_syscur['shortname']."</span><span>{$elem_val['doc_usr_name']}</span></span>";
						echo "</a>";
...
if($h__requested_with_ajax || isset($_POST['TemplateCallback'])){
	SidePanelContent(
		$sql,
		$USER,
		$tables->pagefile_info(234,null,"directory"),
		$c__settings['site']['title']." - ".$tables->pagefile_info(234,null,"title"),
		$tables->pagefile_info(233,null,"directory"),
		$c__settings['site']['title']." - ".$tables->pagefile_info(233,null,"title")
	);
	exit;
}
?>
