<?php
function inner($id,$selectedPermission){
	global $sql;
	static $cnt=1;
	if($r=$sql->query("SELECT 
		...
		if($sql->num_rows($r)>0){
			...
			while($row=$sql->fetch_assoc($r)){
				...
				echo "</td>";
				echo "</tr>";
				inner($row['trd_id'],$selectedPermission);
			}
			$cnt--;
		}
	}
...
if(isset($_POST['permission'])){
	if($r=$sql->query("
		...
		while($row=$sql->fetch_assoc($r)){
			...
			echo  "</td>";
			echo  "</tr>";
			inner($row['trd_id'],(int)$_POST['permission']);
		}
	}
	exit;
}
...
if(isset($_POST['method'],$_POST['xtrd'],$_POST['xper']) && $_POST['method']=='update'){
	...
	if($r=$sql->query("INSERT INTO pagefile_permissions (pfp_trd_id,pfp_per_id,pfp_value) VALUES ({$_POST['xtrd']},{$_POST['xper']},$per) ON DUPLICATE KEY UPDATE pfp_value=$per;")){
		echo "true";
		exit;
	}else{
...
<thead><tr><td>#</td><td width="100%">Page name</td><td></td></tr></thead>
<tbody id="jQoutput"></tbody></table>
<script>
