...
if(isset($_POST['glyph'])){
	if($sql->query("UPDATE pagefile SET trd_attrib4=".($_POST['glyph']==""?"NULL":"'{$_POST['glyph']}'")." WHERE trd_id=".((int)$_POST['id'])."")){
		echo '1';
	}else{
		echo '0';
	}
...
if(isset($_POST['color'])){
	if($sql->query("UPDATE pagefile SET trd_attrib5=".($_POST['color']==""?"NULL":"'{$_POST['color']}'")." WHERE trd_id=".((int)$_POST['id'])."")){
		echo '1';
	}else{
		echo '0';
	}
...
<div id="glyph">
	...
	<div id="colorList">
		<span style="background-color:#fff" data-color=""></span><?php 
		$start=0;
		$step=6;
		echo "";
...
function inner($id){
	global $sql;
	static $cnt=1;
	if($r=$sql->query("SELECT trd_id,trd_directory,pfl_value,trd_attrib4,trd_attrib5
						FROM pagefile JOIN pagefile_language ON pfl_trd_id=trd_id AND pfl_lng_id=1  
						WHERE trd_parent=$id AND pfl_value!='-' ORDER BY trd_zorder;")){
		if($sql->num_rows($r)>0){
			$cnt++;
			$clr=255-($cnt*20);
			...
			while($row=$sql->fetch_assoc($r)){
				...
				echo "<td class=\"btn-set\"><button data-id=\"{$row['trd_id']}\" class=\"cssGlyph\">".($row['trd_attrib4']!=null?"&#xe{$row['trd_attrib4']};":"&nbsp;")."</button><button 
				data-id=\"{$row['trd_id']}\" class=\"cssColor\"><span style=\"background-color:#".($row['trd_attrib5']==null?"fff":$row['trd_attrib5'])."\"></span></button></td>";
				if($row['pfl_value']!="-"){
...
<thead><tr><td>#</td><td></td><td width="100%">Page name</td></tr></thead>
<tbody id="jQoutput">
<?php
if($r=$sql->query("
		SELECT trd_directory,trd_id,pfl_value,trd_attrib4,trd_attrib5
		FROM pagefile JOIN pagefile_language ON pfl_trd_id=trd_id AND pfl_lng_id=1  
		WHERE trd_parent=0
		ORDER BY trd_zorder;")){
		while($row=$sql->fetch_assoc($r)){
...
$(document).ready(function(e) {
	...
	$("#glyph").find("span").on('click',function(){
		...
		$.ajax({
			url:"",
			data:{'id':$current.attr("data-id"),'glyph':_glyph},
			type:"POST"
		}).done(function(data){
			if(data=='1'){
	...
	$("#colorList").find("span").on('click',function(){
		...
		$.ajax({
			url:"",
			data:{'id':$current.attr("data-id"),'color':_color},
			type:"POST"
		}).done(function(data){
			if(data=='1'){
				messagesys.success("Updated successfully");
